...
// so a trailing opener can be recognized and dropped. The opener is the same
// text as the closer in every case tracked here, so its extent is start plus
// that length.
type open struct {
	closer string
	start  int
...
func (o open) end() int { return o.start + len(o.closer) }
...
func scanInline(text string, offset, limit int, stack []open) []open {
	for i := offset; i < limit; {
		...
		case '`', '*', '_', '~':
			...
			if inCode(stack) {
				// Nothing is parsed inside a code span, and only a run of the
				// same length closes it — which the pop above already handled.
				continue
			}
			stack = append(stack, open{closer: delim, start: start})
			continue

		case '[':
			if !inCode(stack) {
				stack = append(stack, open{closer: "]", start: i})
			}
		...
		case ']':
			if top := len(stack) - 1; top >= 0 && stack[top].closer == "]" {
				...
				if i+1 < limit && text[i+1] == '(' {
					stack = append(stack, open{closer: ")", start: i + 1})
					i += 2
					continue
				}
			}
...
// inCode reports whether a code span is open. Nothing may be pushed above one,
// so it can only ever be the innermost entry.
func inCode(stack []open) bool {
	top := len(stack) - 1
	return top >= 0 && stack[top].closer[0] == '`'
}
