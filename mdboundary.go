...
// interpreted.
//
// These rules follow CommonMark's block-boundary semantics.
...
func opensConstruct(line string) bool {
	...
	// otherwise, so a paragraph is only safe to cut after once the following
	// line has been seen — which the pending-candidate rule in boundaryOf does.
	return false
}
